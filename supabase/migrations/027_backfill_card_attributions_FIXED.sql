-- FIXED Backfill script for card_attributions
...
BEGIN
  -- Find the unique constraint on card_attributions
  SELECT conname INTO constraint_name
  FROM pg_constraint
  WHERE conrelid = 'card_attributions'::regclass
    AND contype = 'u'
    AND array_length(conkey, 1) = 4; -- 4 columns in the unique constraint
    
  -- If constraint not found, create it with a known name
  IF constraint_name IS NULL THEN
    -- Drop and recreate with explicit name
    ALTER TABLE card_attributions 
      DROP CONSTRAINT IF EXISTS card_attributions_card_id_user_id_source_stack_id_key;
    
    ALTER TABLE card_attributions
      ADD CONSTRAINT card_attributions_card_id_user_id_source_stack_id_key 
      UNIQUE (card_id, user_id, source, stack_id);
      
    constraint_name := 'card_attributions_card_id_user_id_source_stack_id_key';
  END IF;
  
  -- Store constraint name for use in function
  -- We'll use a fixed name approach instead
END $$;

-- Ensure the constraint exists with the expected name
ALTER TABLE card_attributions 
  DROP CONSTRAINT IF EXISTS card_attributions_card_id_user_id_source_stack_id_key;
...
DROP FUNCTION IF EXISTS backfill_card_attributions_dry_run();
