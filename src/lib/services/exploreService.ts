...
export async function getExploreFeed() {
  ...
  const trendingStacqersResult = await cached(
    ...
    async () => {
      ...
      const updateStat = (
        userId: string,
        field: keyof (typeof userStats)[string]
      ) => {
        if (!userStats[userId])
          userStats[userId] = {
            upvotes_received: 0,
      ...
      // Process Collections Votes
      if (collectionVotes?.length) {
        const ids = collectionVotes
        ...
        if (ids.length) {
          ...
          collectionVotes.forEach((v: any) => {
            if (map[v.target_id])
              updateStat(map[v.target_id], "upvotes_received");
          });
        }
      }
      ...
      if (cardVotes?.length) {
        ...
        if (ids.length) {
          ...
          const map: Record<string, string> = {};
          data?.forEach((c: any) => {
            if (c.created_by) map[c.id] = c.created_by;
          });
          cardVotes.forEach((v: any) => {
            if (map[v.target_id])
              updateStat(map[v.target_id], "upvotes_received");
          });
        }
      ...
      recentFollows?.forEach((f: any) => {
        if (f.following_id) updateStat(f.following_id, "followers_increased");
      });
      ...
      if (recentSaves?.length) {
        ...
        if (ids.length) {
          ...
          recentSaves.forEach((s: any) => {
            if (map[s.target_id])
              updateStat(map[s.target_id], "saves_received");
          });
        }
      }
